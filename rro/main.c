...
// 0./vm  1 argumento adresses.txt 2 algorithm_page_table 3 algorithm_tlb
int main(int agrc, char *argv[]) {
    char input_addresses_name[20];
    char algorithm_page_table[5];
    char algorithm_tlb[5];

    FILE *fp, *fp_backstore, *fp_correct_txt;
    ...
    fp = fopen (argv[1], "r");
    if (fp == NULL) {
        printf("O programa não existe, não pode ser aberto ou escontra-se em uma formatação diferente da indicada\n");
        erro = 2;
    } else {
        if (agrc != 4){
            printf("Número de argumentos diferentes do esperado\n");
            erro = 1;
        } else {
            if (strcmp(argv[2], "fifo") != 0 && (strcmp(argv[2], "lru") != 0 )) {
                printf("O argumento relativo ao algoritmo da page table está escrito errado. Permite-se apenas fifo e lru\n");
                erro = 1;
            } else if (strcmp(argv[3], "fifo") != 0 && (strcmp(argv[3], "lru") != 0 )) {
                printf("O argumento relativo ao algoritmo da tlb está escrito errado. Permite-se apenas fifo e lru\n");
                erro = 1; 
            }
        }
    }
    ...
    if (erro == 0) {
        ...
        // Se o correct.txt já exista por causa de uma execução passada, esvazia!
        fp_correct_txt = fopen("correct.txt", "w");
        fclose(fp_correct_txt);
        fp_correct_txt = fopen("correct.txt", "a");
